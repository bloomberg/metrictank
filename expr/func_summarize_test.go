...
func testSummarizeExtendedRequest(t *testing.T) {
	...
	outputSum := [][]models.Series{
		{
			{
				Target:     "summarize(c, \"60s\", \"sum\")",
				QueryPatt:  "summarize(c, \"60s\", \"sum\")",
				QueryFrom:  10,
				QueryTo:    20,
				Interval:   60,
				Datapoints: getCopy(csum),
			},
		},
		{
			{
				Target:     "summarize(c, \"60s\", \"sum\", true)",
				QueryPatt:  "summarize(c, \"60s\", \"sum\", true)",
				QueryFrom:  10,
				QueryTo:    20,
				Interval:   60,
				Datapoints: getCopy(csum),
	...
	outputMax := [][]models.Series{
		{
			{
				Target:     "summarize(c, \"60s\", \"max\")",
				QueryPatt:  "summarize(c, \"60s\", \"max\")",
				QueryFrom:  10,
				QueryTo:    20,
				Interval:   60,
				Datapoints: getCopy(cmax),
			},
		},
		{
			{
				Target:     "summarize(c, \"60s\", \"max\", true)",
				QueryPatt:  "summarize(c, \"60s\", \"max\", true)",
				QueryFrom:  10,
				QueryTo:    20,
				Interval:   60,
				Datapoints: getCopy(cmax),
...
func testSummarizeAlignToFrom(t *testing.T) {
	...
	outputMax := [][]models.Series{
		...
		{
			{
				QueryPatt:  "summarize(c, \"45s\", \"max\", true)",
				QueryFrom:  30,
				QueryTo:    240,
				Interval:   45,
...
func testSummarize(name string, in []models.Series, out []models.Series, intervalString, fn string, alignToFrom bool, t *testing.T) {
	...
	if len(gots) != len(out) {
		t.Fatalf("case %q (%q, %q, %q): len output expected %d, got %d", name, intervalString, fn, alignToFrom, len(out), len(gots))
	}

	for i, got := range gots {
		exp := out[i]
		if got.Target != exp.Target {
			t.Fatalf("case %q (%q, %q, %q): expected target %q, got %q", name, intervalString, fn, alignToFrom, exp.Target, got.Target)
		}
		if len(got.Datapoints) != len(exp.Datapoints) {
			t.Fatalf("case %q (%q, %q, %q): len output expected %#v, got %#v", name, intervalString, fn, alignToFrom, (exp.Datapoints), (got.Datapoints))
		}
		for j, p := range exp.Datapoints {
			...
			t.Fatalf("case %q (%q, %q, %q): output point %d - expected %v got %v", name, intervalString, fn, alignToFrom, j, p, got.Datapoints[j])
		}
	}
}
