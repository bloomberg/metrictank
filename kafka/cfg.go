...
func (k *KafkaNet) Configure(config *sarama.Config) {
	...
	if k.saslEnabled {
		if k.saslMechanism == "SCRAM-SHA-256" {
			...
			log.Fatalf("Failed to reconize saslMechanism: '%s'", k.saslMechanism)
		}
...
func ConfigNet(FlagSet *flag.FlagSet) *KafkaNet {
	...
	FlagSet.StringVar(&kn.saslMechanism, "sasl-mechanism", "", "The SASL mechanism configuration (possible values: SCRAM-SHA-256, SCRAM-SHA-512)")
